...
class CNN(nn.Module):
    ...
    def __init__(self, params, num_colors, loss_fn=None):
        ...
        #   downsample layers
        self.down_conv_layers = nn.ModuleList().cuda()
        in_channels = self.in_channels
        out_channels = 32
        num_filters = self.num_filters
        for layer_idx in range(self.num_layers):
            self.down_conv_layers.append(nn.Conv2d(in_channels, out_channels, 
                                                   self.kernel_size, padding=self.padding))
            self.down_conv_layers.append(nn.BatchNorm2d(num_filters))
            self.down_conv_layers.append(nn.ReLU())
            self.down_conv_layers.append(nn.MaxPool2d(2))
        ...
        self.refactor_layers.append(nn.Conv2d(in_channels, out_channels, self.kernel_size,
                                             padding=self.padding))
        self.refactor_layers.append(nn.BatchNorm2d(num_filters))
        self.refactor_layers.append(nn.ReLU())
        ...
        num_filters = int(num_filters/2)
        for layer_idx in range(self.num_layers):
            self.up_conv_layers.append(nn.Conv2d(in_channels, out_channels, 
                                                 self.kernel_size, padding=self.padding))
            self.up_conv_layers.append(nn.BatchNorm2d(num_filters))
            self.up_conv_layers.append(nn.ReLU())
            self.up_conv_layers.append(nn.Upsample(scale_factor=2))
            in_channels = out_channels
...
def train(params, colors, model=None, report_dir=None):
    ...
    for epoch in range(0, num_epochs):
        ...
        #   save progress
        if (epoch+1) % 50 == 0:
            checkpoint = {'model': CNN(model_params, num_colors),
...
def evaluate(model, inference_image, colors, save_dir=""):
    
    in_image = torch.tensor(np.array(PIL.Image.open(inference_image))).float()            #   height * width * 3
    in_image = in_image.permute((2,0,1)).unsqueeze(dim=0)                   #   1 * 3 * height * width
        
    max_pixel = float(torch.max(in_image).item())
    grey_image = rgb_to_greyscale(in_image, max_pixel).cuda()      #   1 * 1 * height * width
    grey_image = normalize_array(grey_image.cpu().numpy(), minmax=[0, 1], scale_range=[-1, 1])
    grey_image = torch.tensor(grey_image).cuda().float()
    
    output = model(grey_image)
    
    #   plot images
    max_output = output.max(dim=1)[1].unsqueeze(dim=1)                               #   num_samples * 1 * height *width
    output = map_categories_to_rgb(max_output, colors)              #  1 * 3 * height * width
        
    label = normalize_array(in_image.numpy(), minmax=[0, 255], scale_range=[-1, 1])
    label = map_rgb_to_categories(torch.tensor(label), colors)
    label = map_categories_to_rgb(torch.tensor(label).int(), colors)
    plt.imshow(label[0, :, :, :].permute(1, 2, 0))
    plt.show()
    
    in_image = torch.tensor(normalize_array(in_image.numpy(), minmax=[0, 255], scale_range=[-1, 1]))

    path = ""
    if save_dir != "":
        if save_dir[-1] != "/":
            save_dir += "/"
        path = save_dir + "evaluate.png"
        
    plot_image_set(grey_image, output, label, in_image, path=path)
...
def main(model_params, train_params, train_mode=True, inference_image=""):
    ...
    print("Saving params...")
    report_dir = "../train/"+ str(datetime.now()) +"/"
    ...
    if train_mode is True or train_params['load_location'] == "":
        ...
        model = CNN(model_params, num_colors).cuda()
        #   train model
        print("Training...")
...
if __name__ == "__main__":
    ...
    train_params = {'download_dir':               '../data',
                    ...
                    'load_location':              '/h/arnold/Desktop/NN/Colorization/train/16_horses/model_e199.pth',       #   need to match num_colors
